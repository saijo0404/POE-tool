...
describe('atlasTreeStats', () => {
  ...
  it('translates common atlas passives to Traditional Chinese', () => {
    ...
    expect(translateStatToZh(en1)).toContain('精髓');
    expect(translateStatToZh(en1)).toContain('10%');
    ...
    expect(translateStatToZh(en3)).toContain('命運卡');
    ...
    expect(translateStatToZh(en5)).toContain('軍團');
    expect(translateStatToZh(en5)).toContain('+8%');

    const en6 = '15% increased chance for [ContainsAbyss|Abysses] in your Maps to spawn';
    expect(translateStatToZh(en6)).toContain('深淵');
  });

  it('validates MAX_ATLAS_POINTS constant is exactly 138', async () => {
    const { MAX_ATLAS_POINTS } = await import('../constants');
    expect(MAX_ATLAS_POINTS).toBe(138);
  });

  it('translates 100% of all atlas-tree dataset stats without English leftovers', () => {
