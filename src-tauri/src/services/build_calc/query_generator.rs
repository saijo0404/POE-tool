...
pub fn generate_trade_search_query(
    ...
    let trans_name = crate::services::dictionary::lookup_english_base_type(name)
        .unwrap_or_else(|| name.to_string());
    let trans_type = crate::services::dictionary::lookup_english_base_type(type_line)
        .unwrap_or_else(|| type_line.to_string());
    let mut filters_obj = serde_json::json!({});

    if is_unique {
        apply_unique_query_filters(&mut query_obj, &mut filters_obj, &trans_name, &trans_type);
    } else if is_gem {
        apply_gem_query_filters(
            &mut query_obj,
            &mut filters_obj,
            &trans_name,
            &trans_type,
    ...
    if !is_gem && !is_unique {
        let stat_filters = select_candidate_stat_filters(
            slot,
            &trans_type,
            explicit_mods,
            implicit_mods,
...
fn apply_unique_query_filters(
    query_obj: &mut Value,
    filters_obj: &mut Value,
    trans_name: &str,
    trans_type: &str,
) {
    filters_obj["type_filters"] =
        serde_json::json!({ "filters": { "rarity": { "option": "unique" } } });
...
fn apply_gem_query_filters(
    ...
    if let Some(l) = lvl {
        misc["gem_level"] = serde_json::json!({ "min": l });
    }
    if let Some(q) = qual {
        if q > 0 {
            misc["quality"] = serde_json::json!({ "min": q });
        }
    }
    if let Some(c) = corr {
        if c {
            misc["corrupted"] = serde_json::json!({ "option": "true" });
        }
    }
    if !misc.as_object().map(|o| o.is_empty()).unwrap_or(true) {
        filters_obj["misc_filters"] = serde_json::json!({ "filters": misc });
    }
    let target = if !trans_type.is_empty() {
        trans_type
    } else {
        trans_name
    };
    if !target.is_empty() {
        query_obj["type"] = serde_json::json!(target);
    }
}

fn apply_rare_query_filters(
    query_obj: &mut Value,
    filters_obj: &mut Value,
    trans_type: &str,
    rarity: &str,
    ...
    if rarity.eq_ignore_ascii_case("rare") {
        filters_obj["type_filters"] =
            serde_json::json!({ "filters": { "rarity": { "option": "rare" } } });
    }
    if let Some(l) = links {
        if l >= 5 && (slot.contains("Body") || slot.contains("Weapon") || slot.is_empty()) {
            filters_obj["socket_filters"] =
                serde_json::json!({ "filters": { "links": { "min": l } } });
        }
    }
}
