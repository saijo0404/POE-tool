...
fn get_all_item_containers<'a>(
    ...
    if let Some(sub) = it.get("item").filter(|v| v.is_object()) {
        list.push(sub);
    }
    if let Some(sub) = it.get("itemData").filter(|v| v.is_object()) {
        list.push(sub);
    }
    if let Some(sub) = src.get("item").filter(|v| v.is_object()) {
        list.push(sub);
    }
    if let Some(sub) = src.get("itemData").filter(|v| v.is_object()) {
        list.push(sub);
    }
...
fn parse_ninja_build_equipment_item(
    src: &Value,
    it: &Value,
    inv_id: &str,
    name: &str,
...
pub fn extract_property_numeric_value(
    ...
    for p in props {
        ...
        if names
            .iter()
            .any(|&target| p_name.eq_ignore_ascii_case(target))
        {
            if let Some(val_arr) = p["values"]
                .as_array()
                .and_then(|v| v.first())
