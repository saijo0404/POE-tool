    ...
    for line in lines {
        if !in_mods {
            if line.starts_with("Rarity:") {
                ...
                item.item_level = line.replace("Item Level:", "").trim().parse::<i64>().unwrap_or(85);
                ...
                implicits_count = line.replace("Implicits:", "").trim().parse::<usize>().unwrap_or(0);
                in_mods = true;
            } else if !line.starts_with("Quality:") && !line.starts_with("LevelReq:") && !line.starts_with("Unique ID:") {
...
fn fill_item_names(item: &mut ParsedPobItemRaw, headers: &[&str]) {
    ...
    item.type_line = if headers.len() > 1 { headers[1].to_string() } else { item.name.clone() };
    if item.rarity.is_empty() { item.rarity = "Rare".to_string(); }
    if item.item_level == 0 { item.item_level = 85; }
}
