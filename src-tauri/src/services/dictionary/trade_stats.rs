...
pub fn lookup_stat_with_context(clean_line: &str, is_armour: bool, is_weapon: bool) -> Option<StatMatchResult> {
    if clean_line.is_empty() { return None; }

    let mut numbers = Vec::new();
    for cap in NUM_RE.captures_iter(clean_line) {
        if let Some(m) = cap.get(0) {
            if let Ok(num) = m.as_str().parse::<f64>() { numbers.push(num); }
        }
    }
...
fn fallback_substring_search(stats: &[StatDictionaryEntry], normalized: &str, primary_val: Option<f64>) -> Option<StatMatchResult> {
    for entry in stats {
