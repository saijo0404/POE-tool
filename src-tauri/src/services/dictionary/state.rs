...
impl DictionaryState {
    ...
    pub fn init(&mut self) {
        ...
        if let Ok(embedded_items) = serde_json::from_str::<HashMap<String, String>>(EMBEDDED_ITEM_DICT_JSON) {
            for (k, v) in embedded_items { self.item_dict.insert(k, v); }
        }
        ...
        if stat_path.exists() {
            ...
            if !loaded_stats.is_empty() { stats.extend(loaded_stats); }
        }
        ...
        if item_path.exists() {
            ...
            for (k, v) in loaded_items { self.item_dict.insert(k, v); }
        }
    }
    ...
    fn rebuild_stat_indexes(&mut self) {
        ...
        for entry in &entries {
            let is_local = entry.en_text.contains("(Local)") || entry.en_text.contains("(local)") || entry.zh_text.contains("(部分)") || entry.zh_text.contains("(局部)");
    ...
    fn index_local_entry(&mut self, entry: &StatDictionaryEntry, clean_zh: &str, clean_en: &str, is_armour: bool, is_weapon: bool) {
        let prio = entry_priority(&entry.id);
        if !clean_zh.is_empty() {
            let zh_p = normalize_pattern(clean_zh);
            if is_armour && self.stat_armour_local_map.get(&zh_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_armour_local_map.insert(zh_p.clone(), entry.clone());
            }
            if is_weapon && self.stat_weapon_local_map.get(&zh_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_weapon_local_map.insert(zh_p.clone(), entry.clone());
            }
            if self.stat_local_map.get(&zh_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_local_map.insert(zh_p, entry.clone());
            }
        }
        if !clean_en.is_empty() {
            let en_p = normalize_pattern(clean_en);
            if is_armour && self.stat_armour_local_map.get(&en_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_armour_local_map.insert(en_p.clone(), entry.clone());
            }
            if is_weapon && self.stat_weapon_local_map.get(&en_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_weapon_local_map.insert(en_p.clone(), entry.clone());
            }
            if self.stat_local_map.get(&en_p).map_or(true, |e| prio > entry_priority(&e.id)) {
    ...
    fn index_global_entry(&mut self, entry: &StatDictionaryEntry) {
        ...
        if !entry.zh_text.is_empty() {
            ...
            if self.stat_pattern_map.get(&zh_p).map_or(true, |e| prio > entry_priority(&e.id)) {
                self.stat_pattern_map.insert(zh_p, entry.clone());
            }
        }
        if !entry.en_text.is_empty() {
            let en_p = normalize_pattern(&entry.en_text);
            if self.stat_pattern_map.get(&en_p).map_or(true, |e| prio > entry_priority(&e.id)) {
...
fn check_stat_is_armour(entry: &StatDictionaryEntry) -> bool {
    entry.en_text.contains("Energy Shield") || entry.en_text.contains("Armour") || entry.en_text.contains("Evasion")
        || entry.zh_text.contains("能量護盾") || entry.zh_text.contains("護甲") || entry.zh_text.contains("閃避")
}

fn check_stat_is_weapon(entry: &StatDictionaryEntry) -> bool {
    entry.en_text.contains("Physical Damage") || entry.en_text.contains("Attack Speed") || entry.en_text.contains("Critical Strike Chance") || entry.en_text.contains("Accuracy")
        || entry.zh_text.contains("物理傷害") || entry.zh_text.contains("攻擊速度") || entry.zh_text.contains("暴擊率") || entry.zh_text.contains("命中")
}
