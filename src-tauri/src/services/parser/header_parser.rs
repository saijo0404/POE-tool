...
lazy_static! {
    pub static ref RARITY_ZH_RE: Regex = Regex::new(r"稀有度:\s*([^\r\n]+)").unwrap();
    pub static ref RARITY_EN_RE: Regex = Regex::new(r"Rarity:\s*([^\r\n]+)").unwrap();
    pub static ref ITEM_CLASS_ZH_RE: Regex = Regex::new(r"(?:物品種類|物品類別):\s*([^\r\n]+)").unwrap();
    pub static ref ITEM_CLASS_EN_RE: Regex = Regex::new(r"Item Class:\s*([^\r\n]+)").unwrap();
    pub static ref ITEM_LEVEL_ZH_RE: Regex = Regex::new(r"物品等級:\s*(\d+)").unwrap();
    pub static ref ITEM_LEVEL_EN_RE: Regex = Regex::new(r"Item Level:\s*(\d+)").unwrap();
    pub static ref QUALITY_RE: Regex = Regex::new(r"品質:\s*\+?(\d+)%|Quality:\s*\+?(\d+)%").unwrap();
    pub static ref SOCKETS_RE: Regex = Regex::new(r"(?:插槽|Sockets):\s*([RGBWAW\s-]+)").unwrap();
    pub static ref CORRUPTED_RE: Regex = Regex::new(r"已汙染|已污染|Corrupted").unwrap();
}
...
fn extract_name_and_base(first_section: Option<&&str>) -> (String, String) {
...
fn check_is_armour(class_str: &str, base_type: &str) -> bool {
...
fn check_is_weapon(class_str: &str, base_type: &str) -> bool {
