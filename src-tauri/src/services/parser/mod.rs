...
use header_parser::{
    check_is_armour, check_is_weapon, extract_header_info, extract_rarity,
    CORRUPTED_RE, ITEM_CLASS_EN_RE, ITEM_CLASS_ZH_RE,
    ITEM_LEVEL_EN_RE, ITEM_LEVEL_ZH_RE, QUALITY_RE, SOCKETS_RE,
};
...
fn parse_pob_or_stream_item(clean_text: &str, is_zh: bool, language: &str) -> ParsedItem {
    let all_lines: Vec<&str> = clean_text.lines().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
    if all_lines.is_empty() {
        return ParsedItem {
            name: String::new(), base_type: String::new(), rarity: "Rare".to_string(),
            item_class: None, item_level: None, quality: None, corrupted: None, sockets: None,
            language: language.to_string(), implicits: Vec::new(), explicits: Vec::new(),
            raw_text: clean_text.to_string(),
        };
    }
    ...
    for line in &all_lines {
        ...
        if lower.starts_with("implicits:") {
            if let Some(cnt) = lower.split("implicits:").nth(1).and_then(|s| s.trim().parse::<usize>().ok()) {
    ...
    let (mut name, mut base_type) = if header_candidates.len() >= 2 {
        (header_candidates[0].to_string(), header_candidates[1].to_string())
...
fn is_ignorable_line(line: &str) -> bool {
    let lower = line.to_lowercase();
    if lower.starts_with("備註:") || lower.starts_with("note:") || lower.starts_with("~b/o") || lower.starts_with("~price") || lower.starts_with("unique id:") {
        return true;
    }
    lower.starts_with("需求:") || lower.starts_with("requirements:") ||
    lower.starts_with("等級:") || lower.starts_with("level:") || lower.starts_with("levelreq:") ||
    lower.starts_with("力量:") || lower.starts_with("strength:") || lower.starts_with("str:") ||
    lower.starts_with("敏捷:") || lower.starts_with("dexterity:") || lower.starts_with("dex:") ||
    lower.starts_with("智慧:") || lower.starts_with("intelligence:") || lower.starts_with("int:") ||
    lower.starts_with("物品等級:") || lower.starts_with("item level:") || lower.starts_with("itemlevel:") || lower.starts_with("ilvl:") ||
    lower.starts_with("插槽:") || lower.starts_with("sockets:") ||
    lower.starts_with("護甲:") || lower.starts_with("閃避值:") || lower.starts_with("能量護盾:") || lower.starts_with("格擋機率:") || lower.starts_with("格擋率:") || lower.starts_with("無形性:") ||
    lower.starts_with("armour:") || lower.starts_with("evasion:") || lower.starts_with("energy shield:") || lower.starts_with("ward:") ||
    ...
    lower.contains("塑者之物") || lower.contains("尊師之物") || lower.contains("shaper item") || lower.contains("elder item")
}
