...
pub fn build_trade_headers(settings: &AppSettings, league: &str, query_id: Option<&str>) -> HeaderMap {
...
pub async fn execute_search_http(
    ...
    let search_res = client.post(&search_url).headers(search_headers).json(search_payload).send().await
    ...
    handle_search_error(client, search_payload, target_league, settings, has_auth, search_res).await
}
...
async fn handle_search_error(
    ...
    if status_code.as_u16() == 400 && err_text.contains("Unknown stat provided:") {
        return retry_without_unknown_stat(client, search_payload, target_league, settings, has_auth, &err_text).await;
    }
    ...
    Err(format!("官方市集搜尋回傳錯誤 ({}): {}", status_code, err_text))
}
...
async fn retry_without_unknown_stat(
    ...
    let bad_stat = err_text.split("Unknown stat provided:").nth(1)
    ...
    let search_url = format!("https://www.pathofexile.com/api/trade/search/{}", urlencoding::encode(target_league));
    let search_headers = build_trade_headers(settings, target_league, None);
    let retry_res = client.post(&search_url).headers(search_headers).json(&retry_payload).send().await
        .map_err(|e| format!("PoE Trade API 連線失敗: {}", e))?;
    ...
    if retry_res.status().is_success() {
        ...
        Err(format!("官方市集搜尋回傳錯誤: {}", retry_res.text().await.unwrap_or_default()))
    }
}
...
async fn execute_standard_fallback(
    ...
    let fallback_res = client.post(fallback_url).headers(fallback_headers).json(search_payload).send().await
        .map_err(|e| format!("PoE Trade API 連線失敗: {}", e))?;
    ...
    if fallback_res.status().is_success() {
        ...
        Err(format!("官方市集搜尋回傳錯誤: {}", fallback_res.text().await.unwrap_or_default()))
    }
}
...
pub async fn fetch_listings_http(
    ...
    let fetch_res = client.get(&fetch_url).headers(fetch_headers).send().await
