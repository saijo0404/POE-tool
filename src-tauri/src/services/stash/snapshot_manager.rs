...
use super::stash_api::{fetch_character_items_raw, fetch_stash_tabs_meta, fetch_tab_items_raw, fetch_user_characters};
...
pub fn init_stash_service() {
    ...
    if let Ok(mut guard) = SNAPSHOTS.write() { *guard = loaded; }
}
...
fn set_stash_progress(p: StashProgress) {
    if let Ok(mut guard) = STASH_PROGRESS.write() { *guard = p; }
}
...
pub async fn create_snapshot() -> Result<WealthSnapshot, String> {
    ...
    let ninja_data = crate::services::ninja::fetch_ninja_prices(&target_league, false).await.unwrap_or_else(|_| {
    ...
    if !settings.poesessid.trim().is_empty() && !settings.account_name.trim().is_empty() {
        collect_inventory_items(&mut all_items, &settings, &target_league, &characters, &ninja_data.rates, ninja_data.divine_chaos_rate).await;
        collect_stash_tab_items(&mut all_items, &settings, &target_league, &ninja_data.rates, ninja_data.divine_chaos_rate).await;
    }
    set_stash_progress(StashProgress::default());

    let (total_chaos, total_divine, tab_summaries) = calculate_tab_summaries(&all_items, ninja_data.divine_chaos_rate);
...
fn determine_target_league(settings: &AppSettings, characters: &[serde_json::Value]) -> String {
    if settings.league != "Auto" && !settings.league.is_empty() { return settings.league.clone(); }
    characters.first().and_then(|c| c["league"].as_str()).unwrap_or("Standard").to_string()
}
    ...
    if let Some(char_obj) = char_opt {
        if let Some(c_name) = char_obj["name"].as_str() {
            set_stash_progress(StashProgress {
                active: true, current_tab: 0, total_tabs: 10,
                current_tab_name: format!("角色: {}", c_name), stage: "inventory".to_string(),
            });
            let items = fetch_character_items_raw(settings.account_name.trim(), c_name, settings).await;
            for it in &items {
                if let Some(stash_it) = parse_stash_item(it, &format!("角色裝備與身上 ({})", c_name), rates, div_rate) {
...
async fn collect_stash_tab_items(
    ...
    let tabs_to_fetch: Vec<&crate::models::stash::StashTabMeta> = tabs_meta.iter()
        .take(max_tabs)
    ...
    for (processed, tab) in tabs_to_fetch.iter().enumerate() {
        set_stash_progress(StashProgress {
            active: true, current_tab: processed + 1, total_tabs,
            current_tab_name: tab.n.clone(), stage: "tabs".to_string(),
        });
        let items = fetch_tab_items_raw(settings.account_name.trim(), target_league, tab.i, settings).await;
        for it in &items {
            if let Some(stash_it) = parse_stash_item(it, &format!("倉庫: {}", tab.n), rates, div_rate) {
                all_items.push(stash_it);
            }
        }
    }
