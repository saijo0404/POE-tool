...
pub fn parse_stash_item(
    ...
    let unit_chaos = rates
        .get(type_line)
        .or_else(|| rates.get(name))
        .copied()
        .unwrap_or(0.0);
...
fn categorize_item(it: &Value, type_line: &str) -> &'static str {
    if it["frameType"].as_i64() == Some(5)
        || type_line.contains("Orb")
        || type_line.contains("Chisel")
        || type_line.contains("Mirror")
    {
        "Currency"
    } else if type_line.contains("Fragment")
        || type_line.contains("Key")
        || type_line.contains("Simulacrum")
    {
        "Fragment"
    } else if type_line.contains("Essence") {
        "Essence"
    } else if type_line.contains("Card")
        || type_line.contains("Doctor")
        || type_line.contains("Fiend")
    {
        "DivCard"
