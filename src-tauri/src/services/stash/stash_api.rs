use super::stash_headers::{build_stash_headers, get_settings};
...
pub async fn fetch_stash_tabs_meta(
    custom_league: Option<&str>,
) -> Result<Vec<StashTabMeta>, String> {
    let settings = get_settings();
    ...
    let active_league = if target_league == "Auto" || target_league.is_empty() {
        "Standard"
    } else {
        target_league
    };
    ...
    if res.status().is_success() {
        ...
        let list = tabs
            .iter()
            .enumerate()
            .map(|(idx, t)| parse_tab_meta(t, idx))
            .collect();
        Ok(list)
    } else {
        Ok(Vec::new())
...
pub async fn fetch_character_items_raw(
    account_name: &str,
    char_name: &str,
    settings: &AppSettings,
) -> Vec<Value> {
    let headers = match build_stash_headers(settings) {
        Ok(h) => h,
...
pub async fn fetch_tab_items_raw(
    account_name: &str,
    league: &str,
    tab_index: usize,
    settings: &AppSettings,
    ...
    if acquire_channel_slot(RequestChannel::Stash, true)
        .await
        .is_ok()
    {
        if let Ok(res) = client.get(&url).headers(headers).send().await {
