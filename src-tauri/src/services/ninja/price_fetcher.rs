...
pub async fn fetch_ninja_prices(league: &str, force_refresh: bool) -> Result<NinjaPricesResult, String> {
    ...
    if !force_refresh {
        if let Ok(guard) = NINJA_CACHE.read() {
            if let Some((rates, div_rate, ts)) = guard.get(&active_league) {
                if now_secs() - ts < 1800 {
                    return Ok(NinjaPricesResult { rates: rates.clone(), divine_chaos_rate: *div_rate, league: active_league });
                }
            }
        }
    }
    ...
    let client = reqwest::Client::builder().user_agent(DEFAULT_USER_AGENT).timeout(std::time::Duration::from_secs(15)).build().map_err(|e| e.to_string())?;
    ...
    crate::app_log!("[poe.ninja] 🌐 正在獲取聯盟 '{}' 即時市場物價庫 (PoE1 最新端點)...", query_league);
    fetch_all_ninja_categories(&client, &query_league, &mut rates, &mut divine_chaos_rate, has_live_rate).await;
    ...
    if let Ok(mut guard) = NINJA_CACHE.write() {
        guard.insert(active_league.clone(), (rates.clone(), divine_chaos_rate, now_secs()));
    }

    Ok(NinjaPricesResult { rates, divine_chaos_rate, league: active_league })
}
...
async fn fetch_all_ninja_categories(
    ...
    let exchange_types = ["Currency", "Fragment", "DivinationCard", "Scarab", "Essence", "Oil", "Tattoo", "Omen", "Artifact", "DeliriumOrb"];
    for cat in exchange_types {
        fetch_exchange_overview(client, active_league, cat, rates, divine_chaos_rate, has_live_rate).await;
    }

    let item_types = [
        "UniqueArmour", "UniqueWeapon", "UniqueAccessory", "UniqueFlask", "UniqueJewel", "SkillGem", "ClusterJewel"
    ];
