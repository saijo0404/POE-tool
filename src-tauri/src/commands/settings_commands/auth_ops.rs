...
pub async fn test_connection(
    ...
    if let Some(sess) = poesessid { current.poesessid = sanitize_poesessid(&sess); }
    if let Some(acc) = account_name { current.account_name = acc.trim().to_string(); }
...
pub async fn handle_auto_login_success(
    ...
    if !account_name.trim().is_empty() { settings.account_name = account_name.clone(); }
    if let Some(sess) = poesessid {
        if !sess.trim().is_empty() { settings.poesessid = sess.trim().to_string(); }
    }
    if let Some(cf) = cf_clearance {
        if !cf.trim().is_empty() { settings.cf_clearance = Some(cf.trim().to_string()); }
    }
