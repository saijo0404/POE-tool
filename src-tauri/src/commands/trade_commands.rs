...
use crate::services::trade::{search_trade as search_trade_service, send_official_whisper as send_whisper_service};
...
pub fn parse_item(item_text: String) -> Result<ParsedItem, String> {
    crate::app_log!("[Tauri Command] 📥 parse_item received (len: {} chars):\n{}", item_text.len(), item_text);
    ...
    for (i, m) in parsed.implicits.iter().enumerate() {
        crate::app_log!("   - [Implicit {}] enabled={}, id='{}', text='{}', val={:?}, min={:?}", i, m.enabled, m.id, m.text, m.value, m.min_value);
    }
    for (i, m) in parsed.explicits.iter().enumerate() {
        crate::app_log!("   - [Explicit {}] enabled={}, id='{}', text='{}', val={:?}, min={:?}", i, m.enabled, m.id, m.text, m.value, m.min_value);
    }
    Ok(parsed)
}
...
pub async fn search_trade(request: TradeQueryRequest) -> Result<TradeSearchResult, String> {
    ...
    if let Some(mods) = &request.selected_mods {
        let enabled_count = mods.iter().filter(|m| m.enabled).count();
        crate::app_log!("[Tauri Command] 🔍 search_trade received {} total mods ({} enabled):", mods.len(), enabled_count);
        for (i, m) in mods.iter().enumerate() {
            crate::app_log!("   - Mod [{}]: enabled={}, id='{}', text='{}', min={:?}, max={:?}", i, m.enabled, m.id, m.text, m.min_value, m.max_value);
        }
    } else {
        crate::app_log!("[Tauri Command] ⚠️ search_trade received NO selected_mods (None)");
    }
    search_trade_service(request).await
}

#[tauri::command]
pub async fn send_official_whisper(token: String, league: Option<String>) -> Result<String, String> {
...
pub async fn travel_to_hideout(
    ...
    let message = if game_triggered && official_whisper_sent {
        format!("⚡ 官方直購 (Travel to Hideout) 已觸發，並在遊戲中執行 {}！", hideout_cmd)
    } else if official_whisper_sent {
        format!("⚡ 官方直購 (Travel to Hideout) 已觸發！已複製 {}", hideout_cmd)
...
pub fn open_browser_url(app: Option<&tauri::AppHandle>, url: &str) {
    ...
    if let Some(app_handle) = app {
        if tauri_plugin_shell::ShellExt::shell(app_handle).open(url, None).is_ok() {
            opened = true;
        }
    }
    if !opened {
...
#[tauri::command]
pub async fn create_trade_search_url(app: tauri::AppHandle, league: String, query_json: String) -> Result<String, String> {
    crate::app_log!("[Trade] 🔍 正在向 GGG 官方市集註冊搜尋條件 (聯盟: {})...", league);
    ...
    let api_url = format!("https://www.pathofexile.com/api/trade/search/{}", urlencoding::encode(&league));
    let mut req = client.post(&api_url)
        .header("Origin", "https://www.pathofexile.com")
        .header("Referer", format!("https://www.pathofexile.com/trade/search/{}", urlencoding::encode(&league)))
            ...
            if status.is_success() {
                if let Ok(resp_json) = res.json::<serde_json::Value>().await {
                    if let Some(id) = resp_json["id"].as_str() {
                        let full_url = format!("https://www.pathofexile.com/trade/search/{}/{}", urlencoding::encode(&league), id);
                        crate::app_log!("[Trade] ✅ 成功建立 GGG 官方市集搜尋 ID: {} -> {}", id, full_url);
                        open_browser_url(Some(&app), &full_url);
                        return Ok(full_url);
                    }
                }
            } else {
                crate::app_log!("[Trade] ⚠️ GGG 官方市集註冊查詢回傳狀態: {}", status);
            }
        }
    }

    // Fallback: 如果 GGG API 註冊失敗，直接開啟該聯盟的搜尋首頁
    let fallback_url = format!("https://www.pathofexile.com/trade/search/{}", urlencoding::encode(&league));
