...
pub fn parse_item(item_text: String) -> Result<ParsedItem, String> {
    crate::app_log!("[Tauri Command] 📥 parse_item received (len: {} chars)", item_text.len());
...
pub async fn travel_to_hideout(
    ...
    let hideout_cmd = if let Some(ref char_name) = character_name {
        if !char_name.trim().is_empty() { format!("/hideout {}", char_name.trim()) } else { "/hideout".to_string() }
    } else {
        "/hideout".to_string()
    };
    ...
    if let Some(tok) = token {
        if !tok.trim().is_empty() {
            match send_whisper_service(&tok, league.as_deref()).await {
                Ok(_) => { official_whisper_sent = true; }
                Err(e) => { whisper_error = Some(e); }
            }
        }
    }
    ...
    Ok(TravelResult { success: true, game_triggered, official_whisper_sent, hideout_cmd, message })
}
...
fn open_system_browser_fallback(url: &str) {
    #[cfg(target_os = "windows")]
    {
        ...
        let _ = std::process::Command::new("cmd").args(["/C", "start", "", url]).creation_flags(CREATE_NO_WINDOW).spawn();
    }
    #[cfg(target_os = "linux")]
...
pub async fn create_trade_search_url(app: tauri::AppHandle, league: String, query_json: String) -> Result<String, String> {
    ...
    crate::app_log!("[Trade] 🔍 正在向 GGG 官方市集註冊搜尋條件 (聯盟: {})...", active_league);
    ...
    let api_url = format!("https://www.pathofexile.com/api/trade/search/{}", urlencoding::encode(&active_league));
    let mut req = client.post(&api_url)
        .header("Origin", "https://www.pathofexile.com")
        .header("Referer", format!("https://www.pathofexile.com/trade/search/{}", urlencoding::encode(&active_league)))
            ...
            if status.is_success() {
                if let Ok(resp_json) = res.json::<serde_json::Value>().await {
                    if let Some(id) = resp_json["id"].as_str() {
                        let full_url = format!("https://www.pathofexile.com/trade/search/{}/{}", urlencoding::encode(&active_league), id);
                ...
                crate::app_log!("[Trade] ⚠️ GGG 官方市集註冊查詢回傳狀態 {}: {}", status, err_text);
            }
        }
    }

    let fallback_url = format!("https://www.pathofexile.com/trade/search/{}?q={}", urlencoding::encode(&active_league), urlencoding::encode(&query_json));
