...
4. [🚀 後續更新版本詳細規劃 (Detailed Future Roadmap)](#4--後續更新版本詳細規劃-detailed-future-roadmap)
   ...
   - [階段八 (v2.7.0)：地圖歷史掉落熱力圖、通牒命運試煉精算與自訂置頂快捷鍵 (Map Drop Heatmap, Ultimatum EV & Custom Hotkey Pinning)](#階段八-v270地圖歷史掉落熱力圖通牒命運試煉精算與自訂置頂快捷鍵-map-drop-heatmap-ultimatum-ev--custom-hotkey-pinning)
    ...
    - [階段十七 (v3.3.0)：核心體驗重整與工程架構地基 (Core UX Modernization & Engineering Foundation)](#階段十七-v330核心體驗重整與工程架構地基-core-ux-modernization--engineering-foundation)
    - [階段十八 (v3.4.0)：PoE 2 終局輿圖、深淵刷圖與經濟生態 (PoE 2 Endgame Mapping & Economic Ecosystem)](#階段十八-v340poe-2-終局輿圖深淵刷圖與經濟生態-poe-2-endgame-mapping--economic-ecosystem)
    - [階段十九 (v4.0.0)：次世代流派智慧推薦與跨平台生態 (Intelligent Build Engine & Universal Platform)](#階段十九-v400次世代流派智慧推薦與跨平台生態-intelligent-build-engine--universal-platform)
...
- 🏛️ **六角架構與高標準工程品質**：模組化解耦（檔案 $\le 200$ 行、函式 $\le 30$ 行）、嚴格型別、全自動化測試保護（900+ 前端測試與 70+ Rust 測試）與 GitHub 標準協作流程。
- 📜 **語意化版本與發布治理**：嚴格遵循 [SemVer 2.0.0 與版本發布規範](.antigravity/versioning-policy.md)，杜絕微版本膨脹，以完整業務主題 Epic（Feature Pack）為 Minor 推進單位。
...
gantt
    ...
    section v2.10.0 野獸、凋落與探險精算 (已完成)
...
### 階段十一 (v2.10.0)：野獸工藝配方精算、凋落聖油套利模擬與探險先祖出價優化 (Bestiary Beastcrafting, Blight Oil Combinator & Expedition Haggle Optimizer) `[已發布]`

> **核心目標**：深化野獸工藝成本收益與魔物園任務期望精算，提供凋落聖油 3:1 升級套利比對與凋落蔓延圖期望收益，並精算探險四大 NPC 出價談判模型與日誌收益最大化。

#### 📦 交付功能與關鍵項目

1. **🦁 魔物園獵捕效益精算與野獸工藝配方查詢器 (Bestiary Beastcrafting & Mission EV Engine)** `[已交付/已發布]`
   ...
   - 關聯 PR/Issue：[#146](https://github.com/saijo0404/POE-tool/issues/146), [#149](https://github.com/saijo0404/POE-tool/pull/149)
2. **💧 凋落聖油提煉配比與真菌地圖收益期望精算器 (Blight Oil Combinator & Blighted Map EV Forecaster)** `[已交付/已發布]`
   ...
   - 關聯 PR/Issue：[#147](https://github.com/saijo0404/POE-tool/issues/147), [#150](https://github.com/saijo0404/POE-tool/pull/150)
3. **🧭 探險先祖秘寶出價談判最佳化與日誌收益精算器 (Expedition Haggle Optimizer & Logbook EV Forecaster)** `[已交付/已發布]`
   ...
   - 關聯 PR/Issue：[#148](https://github.com/saijo0404/POE-tool/issues/148), [#151](https://github.com/saijo0404/POE-tool/pull/151)

---

### 階段十二 (v2.11.0)：核心架構收斂、技術債清理與 UI 原語重構 (Core Refactoring, Tech Debt Cleanup & UI Primitives)

> **核心目標**：在邁入 v3.0.0 重大版本前，消除專案歷史累積之技術債，達成 100% 測試檔案合規、解耦根元件路由與事件處理，並建立統一的 UI 設計系統元件庫。
...
### 階段十三 (v2.12.0)：PoE 1 終極架構收斂與後端模組化 (PoE 1 Final Convergence & Rust Modularization)

> **核心目標**：在正式開闢 v3.0.0 (PoE 2) 雙核心架構前，全面清償 Rust 後端服務與測試套件累積之技術債，普及 UI 設計系統原子原語至全站元件，並消除所有遺留之弱型別與安全性警告，打造 100% 健全之 PoE 1 終極 LTS 穩定版。
...
1. **🎮 雙引擎狀態機與進程偵測抽象架構 (Game Engine Context & Process Detection)** `[已交付/已發布]`
   - 建立 `GameEngine` 領域模型 (`'poe1' | 'poe2'`) 與 `useGameEngineStore`。
...
4. **🔨 符文工藝雕刻精算與黑市拍賣行即時行情 (Rune Crafting Forecaster & Black Market Arbitrage)** `[規劃中]`
   ...
   - 對接黑市拍賣行數據，計算金幣 (Gold) 動態兌換比率與套利空間。

---

### 階段十九 (v4.0.0)：次世代流派智慧推薦與跨平台生態 (Intelligent Build Engine & Universal Platform) `[未來展望]`

> **核心目標**：引進啟發式神經網路流派裝備契合度推薦、全面支援 Steam Deck / 移動端 HUD 與社群雲端即時同步。

#### 📦 交付功能與關鍵項目

1. **🧠 AI 流派配裝智慧推薦與屬性瓶頸診斷 (AI Build Optimizer & Gear Diagnostic)** `[未來展望]`
   - 根據角色目前的天賦樹、珠寶插槽與抗性/傷害面板，智慧推薦性價比最高之替換裝備與市集搜尋條件。
2. **🎮 跨平台深度適配與行動遠端儀表板 (Universal Platform & Remote Companion)** `[未來展望]`
   - Steam Deck / Linux 專屬手把操作 HUD 模式與快速輪盤。
   - 手機/平板端本機區域網路即時檢視倉庫資產與刷圖時薪。
